...
export default function ReyDelAroma() {
  ...
  useEffect(() => {
    // ¿Se entró por la RUTA LIMPIA del panel? (reydelaroma.com/<clave>)
    // Si es así, dejamos esa dirección en la barra en vez de limpiarla a "/".
    // Esto es lo que permite que Daniel la ponga como ACCESO DIRECTO en el
    // iPhone: al tocar "Añadir a inicio", Safari guarda la dirección que ve en
    // la barra; si la limpiáramos a "/", el ícono abriría la tienda y no el
    // panel. (El enlace viejo ?panel=… sí se limpia, como antes.)
    let adminCleanPath = "";
    try {
      let raw = (window.location.pathname || "").replace(/^\/+|\/+$/g, "");
      raw = decodeURIComponent(raw);
      if (raw === ADMIN_PANEL_KEY) adminCleanPath = "/" + ADMIN_PANEL_KEY;
    } catch { /* ignore */ }

    // Dirección "limpia" con la que arrancamos (el resultado de pago y el panel
    // abierto por el enlace viejo no se muestran en la barra de direcciones).
    const startPath =
    ...
    try {
      if (adminCleanPath) {
        // Panel abierto por su ruta limpia: la tienda queda como "suelo" (para
        // que "atrás" lleve a la tienda, no fuera del sitio) y en la barra se ve
        // la dirección del panel, lista para guardarla como acceso directo.
        window.history.replaceState({ rda: 1, seed: 1 }, "", "/");
        window.history.pushState({ rda: 1 }, "", adminCleanPath);
      } else {
